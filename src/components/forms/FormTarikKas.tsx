...
interface FormTarikKasProps {
  saldoKas: number;
}

export default function FormTarikKas({ saldoKas }: FormTarikKasProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const nama_penarik = formData.get("nama_penarik") as string;
    const disetujui_oleh = formData.get("disetujui_oleh") as string;
    ...
    if (!nama_penarik.trim()) {
      setError("Nama penarik wajib diisi.");
      return;
    }
    if (!disetujui_oleh.trim()) {
      setError("Nama penyetuju wajib diisi.");
      return;
    }

    startTransition(async () => {
      const { tarikKasKegiatan } = await import("@/app/actions/kas");
      const res = await tarikKasKegiatan({
        jumlah,
        keterangan: keterangan.trim(),
        nama_penarik: nama_penarik.trim(),
        disetujui_oleh: disetujui_oleh.trim(),
      });
      if (!res.success) {
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    min="1000"
                    step="1000"
                    placeholder={`Saldo: ${formatRupiah(saldoKas)}`}
                    style={{
                      width: "100%",
                      padding: "10px 12px",
                      borderRadius: "8px",
                ...
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    required
                    placeholder="Contoh: Upah penarik · Biaya rapat"
                    style={{
                      width: "100%",
                      padding: "10px 12px",
                      borderRadius: "8px",
